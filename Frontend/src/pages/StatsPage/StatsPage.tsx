...
export const StatsPage = () => {
    ...
    const headers = ["Browser","BrowserVersion","Ip","Os","Region","Visiting Date"]
                    ...
                    <div className="bg-white rounded-2xl shadow-xl p-16 text-center">
                        <div className="flex flex-col items-center">
                            ...
                            <p className="text-gray-500 max-w-md">
                                Visitor data will appear here once people start visiting your website.
                            </p>
                        </div>
                    </div>
                ) : (
...
const UserDataCont: FC<UserDataContProps> = ({ data, index }) => {
    ...
    return (
        <tr className={`${rowColor} hover:bg-blue-50 transition-colors duration-200`}>
            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500">
                {index}
            </td>
            {Object.entries(data).sort((a,b)=>a[0]>b[0]?1:-1).map(([key, value]) => (
                <td
                    key={key}
